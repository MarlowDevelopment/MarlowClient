package net.mommymarlow.marlowclient.gui.clickgui;

import com.mojang.blaze3d.systems.RenderSystem;
import net.mommymarlow.marlowclient.Marlow;
import net.mommymarlow.marlowclient.client.MarlowClient;
import net.mommymarlow.marlowclient.gui.clickgui.components.ModButton;
...
public class MarlowClickGui extends Screen {

    // Position saving
    private static final MarlowClickGui INSTANCE = new MarlowClickGui();
    // Minecraft reference
    ...
    protected MarlowClickGui() {
        super(Text.of("Marlow Client GUI"));
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        mc.textRenderer.draw(matrices, MarlowClient.getName(), (windowX + 20) / 2, (windowY + 20) / 2, -1);
        matrices.pop();
        ...
        // Draw the categories
        RenderSystem.enableScissor((int) (windowX + 5) * 2, (int) (-windowY + (height / 2) + 80 /*55*/) * 2, window.getScaledWidth() + 40 /*40*/, window.getScaledHeight()-10);

        // Easier to see where is being scissored
        //RenderUtils.fill(matrices, window.getScaledWidth(), window.getScaledHeight(), -window.getScaledWidth(), -window.getScaledHeight(), -1);
        ...
        if (isHovered(windowX + 100 + settingsFieldX, windowY + 60, windowX + 425 + settingsFieldX, windowY + height, mouseX, mouseY)) {
            if (dWheel < 0 && Math.abs(modScrollEnd) + 170 < (ModuleManager.INSTANCE.getModulesByCategory(selectedCategory).size() * 40)) {
                modScrollEnd -= 25;
            }
    ...
    public void close() {
        ...
        // Save current config
        try {
            MarlowClient.selectedConfig.save();
        } catch (IOException e) {
            e.printStackTrace();
        }
        // Reload the selected config
        try {
            MarlowClient.selectedConfig.load();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static MarlowClickGui getINSTANCE() {
        return INSTANCE;
    }
