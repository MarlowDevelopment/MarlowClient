...
public class AutoAnchor extends Module implements Listener {
    ...
    private void onPacketSend(PacketSendEvent e) {
            ...
            }else if (modes.isMode("Packet")){
                if (e.getPacket() instanceof PlayerInteractBlockC2SPacket packet){
                    if (!InventoryUtils.isHolding(Items.RESPAWN_ANCHOR)) return;
                    InventoryUtils.swap(Items.GLOWSTONE);
                    mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, packet.getBlockHitResult());
                    mc.player.swingHand(Hand.MAIN_HAND);
                    if (InventoryUtils.hasItem(Items.TOTEM_OF_UNDYING)) InventoryUtils.swap(Items.TOTEM_OF_UNDYING);
                    else if (InventoryUtils.hasItem(Items.RESPAWN_ANCHOR)) InventoryUtils.swap(Items.RESPAWN_ANCHOR);
                    else if (InventoryUtils.hasItem(Items.NETHERITE_SWORD)) InventoryUtils.swap(Items.NETHERITE_SWORD);
                    else if (InventoryUtils.hasItem(Items.DIAMOND_SWORD)) InventoryUtils.swap(Items.DIAMOND_SWORD);
                    else return;
                }
            }
    }
